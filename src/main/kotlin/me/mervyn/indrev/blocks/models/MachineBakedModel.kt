package me.mervyn.indrev.blocks.models
...
open class MachineBakedModel(val id: String) : UnbakedModel, BakedModel, FabricBakedModel {
    ...
    //don't judge me
    fun isEmissive(sprite: Sprite?) = sprite?.contents?.id?.toString()?.contains("emissive") == true
    ...
    protected fun QuadEmitter.draw(
        ...
        if (emissives.contains(sprite))
            material(MATERIAL)
        val uv =
            if (sprite.contents.width == 16 && sprite.contents.height == 16) MachineTextureUV.FULL
